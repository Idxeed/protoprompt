...
ProtoPrompt `>=0.20.0,<0.21.0`, потому что использует `ContextPlan` и exact
request receipt.

### Из релиза 0.20.0

`protoprompt-cli` 0.20.0 распространяется как wheel и sdist в соответствующем
...
```powershell
python -m pip install "protoprompt[ollama]==0.20.0"
...
```powershell
python -m pip install "git+https://github.com/Idxeed/protoprompt.git@v0.20.0#subdirectory=apps/agent-cli"
```
